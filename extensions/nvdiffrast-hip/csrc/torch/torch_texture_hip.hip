...
static void set_modes(TextureKernelParams &p, int filter_mode,
                      int boundary_mode, int max_mip_level) {
  // Mip and filter modes.
  p.filterMode = filter_mode;
  NVDR_CHECK(p.filterMode >= 0 && p.filterMode < TEX_MODE_COUNT,
             "filter_mode unsupported");
  p.enableMip = (p.filterMode == TEX_MODE_LINEAR_MIPMAP_NEAREST ||
  ...
  NVDR_CHECK(p.boundaryMode >= 0 && p.boundaryMode < TEX_BOUNDARY_MODE_COUNT,
             "boundary_mode unsupported");
}
...
TextureMipWrapper texture_construct_mip(torch::Tensor tex, int max_mip_level,
                                        bool cube_mode) {
  const at::hip::OptionalHIPGuardMasqueradingAsCUDA device_guard(device_of(tex));
  ...
  if (!cube_mode) {
    NVDR_CHECK(tex.sizes().size() == 4 && tex.size(0) > 0 && tex.size(1) > 0 &&
                   tex.size(2) > 0 && tex.size(3) > 0,
               "tex must have shape[>0, >0, >0, >0]");
  } else {
    NVDR_CHECK(tex.sizes().size() == 5 && tex.size(0) > 0 && tex.size(1) == 6 &&
                   tex.size(2) > 0 && tex.size(3) > 0 && tex.size(4) > 0,
               "tex must have shape[>0, 6, >0, >0, >0] in cube map mode");
    NVDR_CHECK(tex.size(2) == tex.size(3),
               "texture shape must be square in cube map mode");
  }
  ...
  for (int i = 1; i <= p.mipLevelMax; i++) {
    ...
    dim3 blockSize =
        getLaunchBlockSize(TEX_FWD_MAX_MIP_KERNEL_BLOCK_WIDTH,
                           TEX_FWD_MAX_MIP_KERNEL_BLOCK_HEIGHT, sz.x, sz.y);
    dim3 gridSize =
        getLaunchGridSize(blockSize, sz.x, sz.y, sz.z * (cube_mode ? 6 : 1));
    p.mipLevelOut = i;

    void *build_func_tbl[3] = {(void *)MipBuildKernel1, (void *)MipBuildKernel2,
                               (void *)MipBuildKernel4};
    NVDR_CHECK_CUDA_ERROR(hipLaunchKernel(
        reinterpret_cast<const void *>(build_func_tbl[channel_div_idx]),
...
torch::Tensor texture_fwd_mip(torch::Tensor tex, torch::Tensor uv,
  ...
  int max_mip_level =
      has_mip_stack ? mip_stack.size() : mip_wrapper.max_mip_level;
  ...
  if (p.enableMip) {
    NVDR_CHECK(
        has_uv_da || has_mip_level_bias,
        "mipmapping filter mode requires uv_da and/or mip_level_bias input");
    NVDR_CHECK(
        has_mip_stack || mip_w.defined(),
        "mipmapping filter mode requires mip wrapper or mip stack input");
  }
  ...
  if (p.enableMip) {
    if (has_mip_stack) {
      nvdr_check_device_vector(
          mip_stack, __func__,
          "(): Mip stack inputs must reside on the correct GPU device");
      nvdr_check_contiguous(mip_stack, __func__,
                            "(): Mip stack inputs must be contiguous tensors");
      nvdr_check_f32(mip_stack, __func__,
                     "(): Mip stack inputs must be float32 tensors");
    } else {
  ...
  if (!cube_mode) {
    NVDR_CHECK(tex.sizes().size() == 4 && tex.size(0) > 0 && tex.size(1) > 0 &&
                   tex.size(2) > 0 && tex.size(3) > 0,
               "tex must have shape[>0, >0, >0, >0]");
    NVDR_CHECK(uv.sizes().size() == 4 && uv.size(0) > 0 && uv.size(1) > 0 &&
                   uv.size(2) > 0 && uv.size(3) == 2,
               "uv must have shape [>0, >0, >0, 2]");
    ...
    NVDR_CHECK(tex.sizes().size() == 5 && tex.size(0) > 0 && tex.size(1) == 6 &&
               ...
               "tex must have shape[>0, 6, >0, >0, >0] in cube map mode");
    NVDR_CHECK(uv.sizes().size() == 4 && uv.size(0) > 0 && uv.size(1) > 0 &&
                   uv.size(2) > 0 && uv.size(3) == 3,
               "uv must have shape [>0, >0, >0, 3] in cube map mode");
    NVDR_CHECK(tex.size(2) == tex.size(3),
               "texture shape must be square in cube map mode");
  ...
  NVDR_CHECK(tex.size(0) == 1 || tex.size(0) == uv.size(0),
             "minibatch size mismatch between inputs tex, uv");
  NVDR_CHECK(p.texWidth <= (1 << TEX_MAX_MIP_LEVEL) &&
                 p.texHeight <= (1 << TEX_MAX_MIP_LEVEL),
  ...
  if (p.enableMip) {
    if (has_uv_da) {
      if (!cube_mode)
        NVDR_CHECK(uv_da.sizes().size() == 4 && uv_da.size(0) == p.n &&
                       uv_da.size(1) == p.imgHeight &&
                       uv_da.size(2) == p.imgWidth && uv_da.size(3) == 4,
                   "uv_da must have shape [minibatch_size, height, width, 4]");
      else
        NVDR_CHECK(uv_da.sizes().size() == 4 && uv_da.size(0) == p.n &&
                       uv_da.size(1) == p.imgHeight &&
                       uv_da.size(2) == p.imgWidth && uv_da.size(3) == 6,
                   "uv_da must have shape [minibatch_size, height, width, 6] "
                   "in cube map mode");
    }
    if (has_mip_level_bias)
      NVDR_CHECK(
          mip_level_bias.sizes().size() == 3 && mip_level_bias.size(0) == p.n &&
              mip_level_bias.size(1) == p.imgHeight &&
              mip_level_bias.size(2) == p.imgWidth,
          "mip_level_bias must have shape [minibatch_size, height, width]");
  }
  ...
  p.mipLevelBias = (p.enableMip && has_mip_level_bias)
                       ? mip_level_bias.data_ptr<float>()
                       : NULL;
  ...
  torch::TensorOptions opts =
      torch::TensorOptions().dtype(torch::kFloat32).device(torch::kCUDA);
  torch::Tensor out =
      torch::empty({p.n, p.imgHeight, p.imgWidth, p.channels}, opts);
  ...
  if (p.enableMip) {
    if (has_mip_stack) {
      ...
      for (int i = 1; i <= p.mipLevelMax; i++) {
        ...
        if (!cube_mode)
          NVDR_CHECK(t.sizes().size() == 4 && t.size(0) == tex.size(0) &&
                         t.size(1) == sz.y && t.size(2) == sz.x &&
                         t.size(3) == p.channels,
                     "mip level size mismatch in custom mip stack");
        else
          NVDR_CHECK(t.sizes().size() == 5 && t.size(0) == tex.size(0) &&
                         t.size(1) == 6 && t.size(2) == sz.y &&
                         t.size(3) == sz.x && t.size(4) == p.channels,
                     "mip level size mismatch in mip stack");
        if (sz.x == 1 && sz.y == 1)
          NVDR_CHECK(i == p.mipLevelMax,
                     "mip level size mismatch in mip stack");
      ...
      int mipOffsets[TEX_MAX_MIP_LEVEL];
      int mipTotal = calculateMipInfo(p, mipOffsets);
      NVDR_CHECK(tex.sizes() == mip_wrapper.texture_size &&
                     cube_mode == mip_wrapper.cube_mode,
                 "mip does not match texture size");
      NVDR_CHECK(mip_w.sizes().size() == 1 && mip_w.size(0) == mipTotal,
                 "wrapped mip tensor size mismatch");
  ...
  // Verify that buffers are aligned to allow float2/float4 operations. Unused
  // pointers are zero so always aligned.
  if (!cube_mode)
    NVDR_CHECK(!((uintptr_t)p.uv & 7), "uv input tensor not aligned to float2");
  if ((p.channels & 3) == 0) {
    for (int i = 0; i <= p.mipLevelMax; i++)
      NVDR_CHECK(!((uintptr_t)p.tex[i] & 15),
                 "tex or mip input tensor not aligned to float4");
    NVDR_CHECK(!((uintptr_t)p.out & 15),
               "out output tensor not aligned to float4");
    NVDR_CHECK(!((uintptr_t)pmip & 15),
               "mip input tensor not aligned to float4");
  }
  if ((p.channels & 1) == 0) {
    for (int i = 0; i <= p.mipLevelMax; i++)
      NVDR_CHECK(!((uintptr_t)p.tex[i] & 7),
                 "tex or mip input tensor not aligned to float2");
    NVDR_CHECK(!((uintptr_t)p.out & 7),
               "out output tensor not aligned to float2");
    NVDR_CHECK(!((uintptr_t)pmip & 7),
               "mip input tensor not aligned to float2");
  }
  if (!cube_mode)
    NVDR_CHECK(!((uintptr_t)p.uvDA & 15),
               "uv_da input tensor not aligned to float4");
  else
    NVDR_CHECK(!((uintptr_t)p.uvDA & 7),
               "uv_da input tensor not aligned to float2");

  // Choose launch parameters for texture lookup kernel.
  dim3 blockSize = getLaunchBlockSize(TEX_FWD_MAX_KERNEL_BLOCK_WIDTH,
                                      TEX_FWD_MAX_KERNEL_BLOCK_HEIGHT,
                                      p.imgWidth, p.imgHeight);
  dim3 gridSize = getLaunchGridSize(blockSize, p.imgWidth, p.imgHeight, p.n);

  // Choose kernel based on filter mode, cube mode, bias-only mode, and
  // datatype.
  ...
  NVDR_CHECK_CUDA_ERROR(
      hipLaunchKernel(reinterpret_cast<const void *>(func_tbl[func_idx]),
                      gridSize, blockSize, args, 0, stream));
...
torch::Tensor texture_fwd(torch::Tensor tex, torch::Tensor uv, int filter_mode,
                          int boundary_mode) {
  torch::Tensor empty_tensor;
  std::vector<torch::Tensor> empty_vector;
  return texture_fwd_mip(tex, uv, empty_tensor, empty_tensor,
                         TextureMipWrapper(), empty_vector, filter_mode,
                         boundary_mode);
}
...
texture_grad_linear_mipmap_linear(torch::Tensor tex, torch::Tensor uv,
                                  torch::Tensor dy, torch::Tensor uv_da,
                                  torch::Tensor mip_level_bias,
                                  TextureMipWrapper mip_wrapper,
                                  std::vector<torch::Tensor> mip_stack,
                                  int filter_mode, int boundary_mode) {
  const at::hip::OptionalHIPGuardMasqueradingAsCUDA device_guard(device_of(tex));
  ...
  NVDR_CHECK(dy.sizes().size() == 4 && dy.size(0) == p.n &&
                 dy.size(1) == p.imgHeight && dy.size(2) == p.imgWidth &&
                 dy.size(3) == p.channels,
             "dy must have shape [minibatch_size, height, width, channels]");
  ...
  if (p.enableMip) {
    if (has_mip_stack) {
      // Custom mip stack supplied. Check that sizes match, assign, construct
      // gradient tensors.
      p.mipLevelMax = max_mip_level;
      for (int i = 1; i <= p.mipLevelMax; i++) {
        torch::Tensor &t = mip_stack[i - 1];
  ...
  if (!cube_mode) {
    NVDR_CHECK(!((uintptr_t)p.uv & 7), "uv input tensor not aligned to float2");
    NVDR_CHECK(!((uintptr_t)p.gradUV & 7),
               "grad_uv output tensor not aligned to float2");
    NVDR_CHECK(!((uintptr_t)p.uvDA & 15),
               "uv_da input tensor not aligned to float4");
    NVDR_CHECK(!((uintptr_t)p.gradUVDA & 15),
               "grad_uv_da output tensor not aligned to float4");
  } else {
    NVDR_CHECK(!((uintptr_t)p.uvDA & 7),
               "uv_da input tensor not aligned to float2");
    NVDR_CHECK(!((uintptr_t)p.gradUVDA & 7),
               "grad_uv_da output tensor not aligned to float2");
  }
  if ((p.channels & 3) == 0) {
    for (int i = 0; i <= p.mipLevelMax; i++) {
      NVDR_CHECK(!((uintptr_t)p.tex[i] & 15),
                 "tex or mip input tensor not aligned to float4");
      NVDR_CHECK(!((uintptr_t)p.gradTex[i] & 15),
                 "grad_tex output tensor not aligned to float4");
    }
    NVDR_CHECK(!((uintptr_t)p.dy & 15),
               "dy input tensor not aligned to float4");
    NVDR_CHECK(!((uintptr_t)pmip & 15),
               "mip input tensor not aligned to float4");
    NVDR_CHECK(!((uintptr_t)pgradMip & 15),
               "internal mip gradient tensor not aligned to float4");
  }
  if ((p.channels & 1) == 0) {
    for (int i = 0; i <= p.mipLevelMax; i++) {
      NVDR_CHECK(!((uintptr_t)p.tex[i] & 7),
                 "tex or mip input tensor not aligned to float2");
      NVDR_CHECK(!((uintptr_t)p.gradTex[i] & 7),
                 "grad_tex output tensor not aligned to float2");
    }
    NVDR_CHECK(!((uintptr_t)p.dy & 7),
               "dy output tensor not aligned to float2");
    NVDR_CHECK(!((uintptr_t)pmip & 7),
               "mip input tensor not aligned to float2");
    NVDR_CHECK(!((uintptr_t)pgradMip & 7),
               "internal mip gradient tensor not aligned to float2");
  }

  // Choose launch parameters for main gradient kernel.
  void *args[] = {&p};
  dim3 blockSize = getLaunchBlockSize(TEX_GRAD_MAX_KERNEL_BLOCK_WIDTH,
                                      TEX_GRAD_MAX_KERNEL_BLOCK_HEIGHT,
                                      p.imgWidth, p.imgHeight);
  dim3 gridSize = getLaunchGridSize(blockSize, p.imgWidth, p.imgHeight, p.n);
  ...
  // Launch kernel to pull gradients from mip levels. Don't do this if mip stack
  // was supplied - individual level gradients are already there.
  if (p.enableMip && !has_mip_stack) {
    dim3 blockSize = getLaunchBlockSize(TEX_GRAD_MAX_MIP_KERNEL_BLOCK_WIDTH,
                                        TEX_GRAD_MAX_MIP_KERNEL_BLOCK_HEIGHT,
                                        p.texWidth, p.texHeight);
    dim3 gridSize = getLaunchGridSize(blockSize, p.texWidth, p.texHeight,
                                      p.texDepth * (cube_mode ? 6 : 1));
    int sharedBytes = blockSize.x * blockSize.y * p.channels * sizeof(float);

    void *mip_grad_func_tbl[3] = {
        (void *)MipGradKernel1, (void *)MipGradKernel2, (void *)MipGradKernel4};
    NVDR_CHECK_CUDA_ERROR(hipLaunchKernel(
        reinterpret_cast<const void *>(mip_grad_func_tbl[channel_div_idx]),
        gridSize, blockSize, args, sharedBytes, stream));
  }

  // Return output tensors.
  return std::tuple<torch::Tensor, torch::Tensor, torch::Tensor, torch::Tensor,
                    std::vector<torch::Tensor>>(
      grad_tex, grad_uv, grad_uv_da, grad_mip_level_bias, grad_mip_stack);
}

// Version for nearest filter mode.
torch::Tensor texture_grad_nearest(torch::Tensor tex, torch::Tensor uv,
  ...
  std::tuple<torch::Tensor, torch::Tensor, torch::Tensor, torch::Tensor,
             std::vector<torch::Tensor>>
      result = texture_grad_linear_mipmap_linear(
          tex, uv, dy, empty_tensor, empty_tensor, TextureMipWrapper(),
          empty_vector, filter_mode, boundary_mode);
  return std::get<0>(result);
}

// Version for linear filter mode.
std::tuple<torch::Tensor, torch::Tensor>
texture_grad_linear(torch::Tensor tex, torch::Tensor uv, torch::Tensor dy,
                    int filter_mode, int boundary_mode) {
  torch::Tensor empty_tensor;
  std::vector<torch::Tensor> empty_vector;
  std::tuple<torch::Tensor, torch::Tensor, torch::Tensor, torch::Tensor,
      ...
      result = texture_grad_linear_mipmap_linear(
          tex, uv, dy, empty_tensor, empty_tensor, TextureMipWrapper(),
          empty_vector, filter_mode, boundary_mode);
  return std::tuple<torch::Tensor, torch::Tensor>(std::get<0>(result),
                                                  std::get<1>(result));
}

// Version for linear-mipmap-nearest mode.
std::tuple<torch::Tensor, torch::Tensor, std::vector<torch::Tensor>>
texture_grad_linear_mipmap_nearest(torch::Tensor tex, torch::Tensor uv,
                                   torch::Tensor dy, torch::Tensor uv_da,
                                   torch::Tensor mip_level_bias,
                                   TextureMipWrapper mip_wrapper,
                                   std::vector<torch::Tensor> mip_stack,
  ...
  std::tuple<torch::Tensor, torch::Tensor, torch::Tensor, torch::Tensor,
             std::vector<torch::Tensor>>
      result =
          texture_grad_linear_mipmap_linear(tex, uv, dy, uv_da, mip_level_bias,
                                            mip_wrapper, mip_stack, filter_mode,
                                            boundary_mode);
  return std::tuple<torch::Tensor, torch::Tensor, std::vector<torch::Tensor>>(
      std::get<0>(result), std::get<1>(result), std::get<4>(result));
}
